<template>
  <b-modal v-model="showModal" header-class="p-3" :title="action + ' BAC Resolution'" size="xl" centered no-close-on-backdrop>
...
export default {
  ...
  methods: {
    show(type) {
      console.log(this.procurement, 888);
      this.showModal = true;
      this.form.type = type;
      this.action = "Create";
      this.Content();
    ...
    Content() {
      ...
      this.reaward_body = `
        <div style=" font-size: 16px;">
          <p style="text-align: justify ;   text-align-last: center;  margin-bottom: 2em; ">
            <b>
              RECOMMENDING FOR RE-AWARD OF CONTRACT TO ${awardedSupplierNames}, AS THE ${calculatedBidderLabelCap} CALCULATED AND RESPONSIVE BIDDER FOR THE PROCUREMENT
              "${this.procurement.title.toUpperCase()}" UNDERTAKEN THROUGH SECTION 53.9 (${modeOfProcurement_names})
              OF THE REVISED IMPLEMENTING RULES AND REGULATIONS OF (${modeOfProcurement_ra_nos})
            </b>
          </p>
    ...
    submit(data) {
      if(this.action == 'Create'){
        ...
        this.form.put('/faims/bac-resolutions/'+data.id);
        this.hide();
